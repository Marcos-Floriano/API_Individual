...
class AnimalServiceTest {
    ...
    fun `deve listar todos os animais`(id: Int, nome: String, especie: String, idade: Int) {
        val animal = Animais(id = id, nome = nome, especie = especie, idade = idade)
    ...
    fun `deve buscar animal por id`(id: Int, nome: String, especie: String, idade: Int) {
        val animal = Animais(id = id, nome = nome, especie = especie, idade = idade)
        `when`(animalRepository.findById(id.toLong())).thenReturn(Optional.of(animal))
    ...
    fun `deve criar um novo animal`() {
        ...
        val animal = Animais(nome = animalDTO.nome, especie = animalDTO.especie, idade = animalDTO.idade)
        `when`(animalRepository.save(any(Animais::class.java))).thenReturn(animal)
